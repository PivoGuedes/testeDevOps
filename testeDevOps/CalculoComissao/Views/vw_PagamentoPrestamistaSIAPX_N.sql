...
--select * from  [CalculoComissao].[vw_PagamentoPrestamistaSIAPX]

CREATE VIEW [CalculoComissao].[vw_PagamentoPrestamistaSIAPX_N]
AS
WITH CTE
AS
(
SELECT  PE.ID
      , CNT.NumeroContrato
	  , RIGHT(P.NumeroProposta,14) NumeroProposta
...
AND	CodigoComercializado <> '7711' and NumeroProposta not	like 'PS%'
--INSERIDO EM 2015-05-19
UNION

SELECT PGTO.ID
     , CNT.NumeroContrato
     , RIGHT(P.NumeroProposta,14) AS NumeroProposta
     , PRD.CodigoComercializado [CodigoProduto]
...
AND	CodigoComercializado = '7711' and NumeroProposta like 'PS%'	
--INSERIDO EM 2015-05-19
UNION
