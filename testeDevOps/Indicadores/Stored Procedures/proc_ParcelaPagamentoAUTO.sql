...
/*******************************************************************************
	Nome: CORPORATIVO.Indicadores.proc_ParcelaPagamentoAUTO
	Descrição: Procedimento que recupera parcelas de AUTO emitidos em um determinado período.
...
*******************************************************************************/
CREATE PROCEDURE [Indicadores].[proc_ParcelaPagamentoAUTO] @DataInicioEmissao date, @DataFimEmissao date
AS

WITH CTE
AS
(
SELECT CNT.ID IDContrato, PRP.IDFuncionario, /*EN.ID,*/   CNT.NumeroContrato,  EN.NumeroEndosso, PRD.CodigoComercializado, PRD.Descricao[Produto], 
                         ...
                         PRP.[DataProposta], PR.IDSituacaoParcela, CNT.DataEmissao--, pr.NomeArquivo
...
--AND cnt.numerocontrato =  '1103100833693'
--AND PR.TipoDado <> 'PRD.FENAE.PARCELA'
--AND SP.Descricao = 'Paga'
AND PO.Descricao = 'GERADO PELA EMISSAO'
AND CNT.IDSeguradora = 1
--order by cnt.NumeroContrato, pr.NumeroParcela
)
--SELECT COUNT(DISTINCT NUMEROCONTRATO)
--FROM CTE
,
CTEA
AS
...
SELECT CTE.IDContrato, COALESCE(CTE.IDFuncionario, CTEA.IDFuncionario, - 1) IDFuncionario, F.Matricula, /*EN.ID,*/   CTE.NumeroContrato, CTE.NumeroEndosso, CTE.CodigoComercializado, CTE.[Produto], 
...
--exec [Indicadores].[proc_ParcelaPagamentoAUTO] '20160801', '20160831'
