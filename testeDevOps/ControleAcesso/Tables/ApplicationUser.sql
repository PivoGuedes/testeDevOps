CREATE TABLE [ControleAcesso].[ApplicationUser] (
    [ID]            INT              IDENTITY (1, 1) NOT NULL,
    [IDApplication] UNIQUEIDENTIFIER NOT NULL,
    [IDUser]        UNIQUEIDENTIFIER NOT NULL,
    CONSTRAINT [PK_ApplicationUser_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE) ON [PRIMARY],
    CONSTRAINT [FK_ApplicationUser_Application_ID] FOREIGN KEY ([IDApplication]) REFERENCES [ControleAcesso].[Application] ([ID]),
    CONSTRAINT [FK_ApplicationUser_User_ID] FOREIGN KEY ([IDUser]) REFERENCES [ControleAcesso].[User] ([ID])
);


GO
ALTER TABLE [ControleAcesso].[ApplicationUser] NOCHECK CONSTRAINT [FK_ApplicationUser_Application_ID];


GO
ALTER TABLE [ControleAcesso].[ApplicationUser] NOCHECK CONSTRAINT [FK_ApplicationUser_User_ID];
...
CREATE TRIGGER [ControleAcesso].[ControleAcesso_ApplicationUser_ChangeTracking] on [ControleAcesso].[ApplicationUser] for insert, update, delete
...
select @TableName = 'ApplicationUser'

select @TableSchema = 'ControleAcesso'

SELECT @TableName_Full = '[ControleAcesso].[ApplicationUser]'
...
SELECT [ID],[IDApplication],[IDUser] INTO #ins FROM inserted

SELECT [ID],[IDApplication],[IDUser] INTO #del FROM deleted
