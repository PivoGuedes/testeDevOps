CREATE TABLE [ControleAcesso].[ProfileUser] (
    [ID]        INT              IDENTITY (1, 1) NOT NULL,
    [IDProfile] INT              NOT NULL,
    [IDUser]    UNIQUEIDENTIFIER NOT NULL,
    CONSTRAINT [PK_ProfileUser_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_ProfileUser_Profile_ID] FOREIGN KEY ([IDProfile]) REFERENCES [ControleAcesso].[Profile] ([ID]),
    CONSTRAINT [FK_ProfileUser_User_ID] FOREIGN KEY ([IDUser]) REFERENCES [ControleAcesso].[User] ([ID])
);


GO
ALTER TABLE [ControleAcesso].[ProfileUser] NOCHECK CONSTRAINT [FK_ProfileUser_Profile_ID];


GO
ALTER TABLE [ControleAcesso].[ProfileUser] NOCHECK CONSTRAINT [FK_ProfileUser_User_ID];
...
CREATE TRIGGER [ControleAcesso].[ControleAcesso_ProfileUser_ChangeTracking] on [ControleAcesso].[ProfileUser] for insert, update, delete
...
select @TableName = 'ProfileUser'

select @TableSchema = 'ControleAcesso'

SELECT @TableName_Full = '[ControleAcesso].[ProfileUser]'
...
SELECT [ID],[IDProfile],[IDUser] INTO #ins FROM inserted

SELECT [ID],[IDProfile],[IDUser] INTO #del FROM deleted
