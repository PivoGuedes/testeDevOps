CREATE TABLE [ControleAcesso].[User] (
    ...
    [FailedPasswordAttempt]   INT              NOT NULL,
    [CreationDate]            DATETIME         NOT NULL,
    [LastAttempt]             DATETIME         NULL,
    [LastLogin]               DATETIME         NULL,
    [EnableChangePassword]    BIT              NULL,
    [RequiresActivateAccount] BIT              NOT NULL,
    [AuthenticationMode]      SMALLINT         NOT NULL,
    [Matricula]               VARCHAR (20)     NULL,
    CONSTRAINT [PK_User_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE) ON [PRIMARY]
);
...
CREATE TRIGGER [ControleAcesso].[ControleAcesso_User_ChangeTracking] on [ControleAcesso].[User] for insert, update, delete
...
SELECT @TableName_Full = '[ControleAcesso].[User]'
...
SELECT [ID],[Login],[Name],[Email],[Password],[PasswordAnswer],[PasswordQuestion],[Token],[IsDisabled],[IsLocked],[FailedPasswordAttempt],[CreationDate],[LastAttempt],[LastLogin],[EnableChangePassword],[RequiresActivateAccount],[AuthenticationMode],[Matricula] INTO #ins FROM inserted

SELECT [ID],[Login],[Name],[Email],[Password],[PasswordAnswer],[PasswordQuestion],[Token],[IsDisabled],[IsLocked],[FailedPasswordAttempt],[CreationDate],[LastAttempt],[LastLogin],[EnableChangePassword],[RequiresActivateAccount],[AuthenticationMode],[Matricula] INTO #del FROM deleted
