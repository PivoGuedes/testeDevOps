CREATE TABLE [Dados].[SituacaoProposta] (
    [ID]                TINYINT       IDENTITY (1, 1) NOT NULL,
    [Sigla]             CHAR (3)      NOT NULL,
    [Descricao]         VARCHAR (100) NOT NULL,
    [SituacaoDeEmissao] BIT           CONSTRAINT [DF_SituacaoProposta_SituacaoDeEmissao] DEFAULT ((0)) NOT NULL,
    CONSTRAINT [PK_SituacaoProposta_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_SituacaoProposta_Sigla] UNIQUE NONCLUSTERED ([Sigla] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_SituacaoProposta_ChangeTracking] on [Dados].[SituacaoProposta] for insert, update, delete
...
select @TableName = 'SituacaoProposta'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[SituacaoProposta]'
...
SELECT [ID],[Sigla],[Descricao], SituacaoDeEmissao INTO #ins FROM inserted

SELECT [ID],[Sigla],[Descricao], SituacaoDeEmissao INTO #del FROM deleted
...
CREATE TRIGGER [Dados].[trg_SendDBMail_SituacaoProposta]
ON [Dados].[SituacaoProposta]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.SituacaoProposta, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Sigla ' + ISNULL(CAST(@Sigla AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
