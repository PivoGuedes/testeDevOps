CREATE TABLE [Dados].[PeriodoPagamento] (
    [ID]        TINYINT      IDENTITY (1, 1) NOT NULL,
    [Codigo]    CHAR (2)     NULL,
    [Descricao] VARCHAR (20) NOT NULL,
    CONSTRAINT [PK_PeriodoPagamento_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_PeriodoPagamento] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_PeriodoPagamento_ChangeTracking] on [Dados].[PeriodoPagamento] for insert, update, delete
...
select @TableName = 'PeriodoPagamento'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[PeriodoPagamento]'
