CREATE TABLE [Dados].[FilialPARCorretora] (
    [ID]     SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Codigo] VARCHAR (14)  NULL,
    [Nome]   VARCHAR (100) NULL,
    CONSTRAINT [PK_FILIALPARCORRETORA] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQFilialPARCorretoraCodigo] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQFilialPARCorretoraNome] UNIQUE NONCLUSTERED ([Nome] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);


GO

CREATE TRIGGER [Dados].[trg_SendDBMail_FilialPARCorretora]
ON [Dados].[FilialPARCorretora]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.FilialPARCorretora, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Código ' + ISNULL(CAST(@Codigo AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
...
CREATE TRIGGER [Dados].[Dados_FilialPARCorretora_ChangeTracking] on [Dados].[FilialPARCorretora] for insert, update, delete
...
select @TableName = 'FilialPARCorretora'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[FilialPARCorretora]'
...
SELECT [ID],[Codigo],[Nome] INTO #ins FROM inserted

SELECT [ID],[Codigo],[Nome] INTO #del FROM deleted
