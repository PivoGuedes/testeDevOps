CREATE TABLE [Dados].[CanalVenda] (
    [ID]          TINYINT      NOT NULL,
    [CodigoSIGPF] SMALLINT     NULL,
    [Nome]        VARCHAR (50) NULL,
    CONSTRAINT [PK_CANALVENDA] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_CanalVenda_ChangeTracking] on [Dados].[CanalVenda] for insert, update, delete
...
select @TableName = 'CanalVenda'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[CanalVenda]'
...
SELECT [ID],[CodigoSIGPF],[Nome] INTO #ins FROM inserted

SELECT [ID],[CodigoSIGPF],[Nome] INTO #del FROM deleted
