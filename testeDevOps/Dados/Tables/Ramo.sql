CREATE TABLE [Dados].[Ramo] (
    [ID]          SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Codigo]      VARCHAR (4)   NULL,
    [IDGrupoRamo] TINYINT       NULL,
    [Nome]        VARCHAR (100) NULL,
    [TipoRamo]    VARCHAR (80)  NULL,
    CONSTRAINT [PK_RAMO] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQRamo] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_Ramo_ChangeTracking] on [Dados].[Ramo] for insert, update, delete
...
select @TableName = 'Ramo'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Ramo]'
...
SELECT [ID],[Codigo],[Nome],[TipoRamo], IDGrupoRamo INTO #ins FROM inserted

SELECT [ID],[Codigo],[Nome],[TipoRamo], IDGrupoRamo INTO #del FROM deleted
