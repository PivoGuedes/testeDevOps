CREATE TABLE [Dados].[TipoVinculo] (
    [ID]   TINYINT      NOT NULL,
    [Nome] VARCHAR (40) NULL,
    CONSTRAINT [PK_TIPOVINCULO] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);


GO

CREATE TRIGGER [Dados].[trg_SendDBMail_TipoVinculo]
ON [Dados].[TipoVinculo]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.TipoVinculo, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'')+ ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
...
CREATE TRIGGER [Dados].[Dados_TipoVinculo_ChangeTracking] on [Dados].[TipoVinculo] for insert, update, delete
...
select @TableName = 'TipoVinculo'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[TipoVinculo]'
...
SELECT [ID],[Nome] INTO #ins FROM inserted

SELECT [ID],[Nome] INTO #del FROM deleted
