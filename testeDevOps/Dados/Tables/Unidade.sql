CREATE TABLE [Dados].[Unidade] (
    [ID]     SMALLINT IDENTITY (1, 1) NOT NULL,
    [Codigo] SMALLINT NOT NULL,
    CONSTRAINT [PK_UNIDADE] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQCODIGO_UNIDADE] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [UNQ_Unidade]
    ON [Dados].[Unidade]([Codigo] ASC)
...
CREATE TRIGGER [Dados].[Dados_Unidade_ChangeTracking] on [Dados].[Unidade] for insert, update, delete
...
select @TableName = 'Unidade'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Unidade]'
...
SELECT [ID],[Codigo] INTO #ins FROM inserted

SELECT [ID],[Codigo] INTO #del FROM deleted
...
GO
DISABLE TRIGGER [Dados].[Dados_Unidade_ChangeTracking]
    ON [Dados].[Unidade];
