CREATE TABLE [Dados].[Profissao] (
    [ID]        SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Descricao] VARCHAR (100) NOT NULL,
    [CodigoCBO] INT           NULL,
    CONSTRAINT [PK_Profissao_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_Profissao_Descricao] UNIQUE NONCLUSTERED ([Descricao] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_Profissao_ChangeTracking] on [Dados].[Profissao] for insert, update, delete
...
select @TableName = 'Profissao'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Profissao]'
...
SELECT [ID],[Descricao],[CodigoCBO] INTO #ins FROM inserted

SELECT [ID],[Descricao],[CodigoCBO] INTO #del FROM deleted
