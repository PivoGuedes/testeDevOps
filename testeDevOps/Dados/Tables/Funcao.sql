CREATE TABLE [Dados].[Funcao] (
    [ID]        SMALLINT     IDENTITY (1, 1) NOT NULL,
    [Codigo]    VARCHAR (6)  NULL,
    [Descricao] VARCHAR (70) NOT NULL,
    CONSTRAINT [PK_Funcao] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);


GO
CREATE NONCLUSTERED INDEX [IDX_NCL_Funcao]
    ON [Dados].[Funcao]([Codigo] ASC)
...
CREATE TRIGGER [Dados].[Dados_Funcao_ChangeTracking] on [Dados].[Funcao] for insert, update, delete
...
select @TableName = 'Funcao'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Funcao]'
