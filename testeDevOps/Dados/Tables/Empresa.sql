CREATE TABLE [Dados].[Empresa] (
    [ID]                  SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Nome]                VARCHAR (100) NOT NULL,
    [Descricao]           VARCHAR (400) NOT NULL,
    [Agrupamento]         VARCHAR (100) NULL,
    [CodigoEmpresaPROPAY] VARCHAR (10)  NULL,
    CONSTRAINT [PK_Empresa_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_Empresa_ChangeTracking] on [Dados].[Empresa] for insert, update, delete
...
select @TableName = 'Empresa'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Empresa]'
...
SELECT [ID],[Nome],[Descricao],[Agrupamento], CodigoEmpresaPROPAY INTO #ins FROM inserted

SELECT [ID],[Nome],[Descricao],[Agrupamento],CodigoEmpresaPROPAY INTO #del FROM deleted
