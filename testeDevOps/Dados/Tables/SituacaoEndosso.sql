CREATE TABLE [Dados].[SituacaoEndosso] (
    [ID]        TINYINT      NOT NULL,
    [Codigo]    TINYINT      NULL,
    [Descricao] VARCHAR (20) NULL,
    CONSTRAINT [PK_SITUACAOENDOSSO] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_SituacaoEndosso] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);


GO

CREATE TRIGGER [Dados].[trg_SendDBMail_SituacaoEndosso]
ON [Dados].[SituacaoEndosso]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.SituacaoEndosso, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Código ' + ISNULL(CAST(@Codigo AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
...
CREATE TRIGGER [Dados].[Dados_SituacaoEndosso_ChangeTracking] on [Dados].[SituacaoEndosso] for insert, update, delete
...
select @TableName = 'SituacaoEndosso'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[SituacaoEndosso]'
