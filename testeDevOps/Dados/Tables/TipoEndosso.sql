CREATE TABLE [Dados].[TipoEndosso] (
    [ID]              TINYINT      NOT NULL,
    [Codigo]          TINYINT      NULL,
    [Descricao]       VARCHAR (50) NULL,
    [SinalLancamento] SMALLINT     NULL,
    [Cancelamento]    BIT          NULL,
    CONSTRAINT [PK_TIPOENDOSSO] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_TipoEndosso] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_TipoEndosso_ChangeTracking] on [Dados].[TipoEndosso] for insert, update, delete
...
select @TableName = 'TipoEndosso'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[TipoEndosso]'
...
SELECT [ID],[Codigo],[Descricao], SinalLancamento, Cancelamento INTO #ins FROM inserted

SELECT [ID],[Codigo],[Descricao], SinalLancamento, Cancelamento INTO #del FROM deleted
...
CREATE TRIGGER [Dados].[trg_SendDBMail_TipoEndosso]
ON [Dados].[TipoEndosso]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.TipoEndosso, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Código ' + ISNULL(CAST(@Codigo AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
