CREATE TABLE [Dados].[FormaPagamento] (
    [ID]        TINYINT       NOT NULL,
    [Descricao] VARCHAR (100) NOT NULL,
    CONSTRAINT [PK_FormaPagamento_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_FormaPagamento_ChangeTracking] on [Dados].[FormaPagamento] for insert, update, delete
...
select @TableName = 'FormaPagamento'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[FormaPagamento]'
...
GO
CREATE TRIGGER [Dados].[trg_SendDBMail_FormaPagamento]
ON [Dados].[FormaPagamento]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.FormaPagamento, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'')+ ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
