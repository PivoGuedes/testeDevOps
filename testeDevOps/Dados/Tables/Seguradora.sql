CREATE TABLE [Dados].[Seguradora] (
    [ID]              SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Codigo]          VARCHAR (6)   NULL,
    [Nome]            VARCHAR (100) NULL,
    [CodigoSUSEP]     VARCHAR (6)   NULL,
    [FlagMostraNoBox] BIT           NULL,
    CONSTRAINT [PK_SEGURADORA] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_Seguradora] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_Seguradora_ChangeTracking] on [Dados].[Seguradora] for insert, update, delete
...
select @TableName = 'Seguradora'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Seguradora]'
...
SELECT [ID],[Codigo],[Nome], CodigoSUSEP,FlagMostraNoBox INTO #ins FROM inserted

SELECT [ID],[Codigo],[Nome], CodigoSUSEP,FlagMostraNoBox INTO #del FROM deleted
