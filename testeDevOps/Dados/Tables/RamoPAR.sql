CREATE TABLE [Dados].[RamoPAR] (
    [ID]           SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Codigo]       VARCHAR (2)   NULL,
    [Nome]         VARCHAR (100) NULL,
    [IDRamoMestre] SMALLINT      NULL,
    CONSTRAINT [PK_RAMOPAR] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_RAMOPARID_RAMOPARIDMESTRE] FOREIGN KEY ([IDRamoMestre]) REFERENCES [Dados].[RamoPAR] ([ID]),
    CONSTRAINT [UNQRamoPAR] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_RamoPAR_ChangeTracking] on [Dados].[RamoPAR] for insert, update, delete
...
select @TableName = 'RamoPAR'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[RamoPAR]'
...
SELECT [ID],[Codigo],[Nome],[IDRamoMestre] INTO #ins FROM inserted

SELECT [ID],[Codigo],[Nome],[IDRamoMestre] INTO #del FROM deleted
