CREATE TABLE [Dados].[TipoUnidade] (
    [ID]        SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Descricao] VARCHAR (200) NULL,
    CONSTRAINT [PK_TipoUnidade_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_TipoUnidade_Descricao] UNIQUE NONCLUSTERED ([Descricao] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_TipoUnidade_ChangeTracking] on [Dados].[TipoUnidade] for insert, update, delete
...
select @TableName = 'TipoUnidade'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[TipoUnidade]'
...
CREATE TRIGGER [Dados].[trg_SendDBMail_TipoUnidade]
ON [Dados].[TipoUnidade]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.TipoUnidade, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'')+ ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
