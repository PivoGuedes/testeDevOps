CREATE TABLE [Dados].[ComissaoOperacao] (
    [ID]               TINYINT      IDENTITY (1, 1) NOT NULL,
    [Codigo]           SMALLINT     NULL,
    [Descricao]        VARCHAR (40) NULL,
    [SinalOperacao]    SMALLINT     NULL,
    [LancamentoManual] BIT          NULL,
    CONSTRAINT [PK_COMISSAOOPERACAO] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_COMISSAOOPERAC_COMISSAO] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_ComissaoOperacao_ChangeTracking] on [Dados].[ComissaoOperacao] for insert, update, delete
...
select @TableName = 'ComissaoOperacao'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[ComissaoOperacao]'
...
SELECT [ID],[Codigo],[Descricao],[SinalOperacao],[LancamentoManual] INTO #ins FROM inserted

SELECT [ID],[Codigo],[Descricao],[SinalOperacao],[LancamentoManual] INTO #del FROM deleted
...
GO
CREATE TRIGGER [Dados].[trg_SendDBMail_ComissaoOperacao]
ON [Dados].[ComissaoOperacao]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.ComissaoOperacao, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Código ' + ISNULL(CAST(@Codigo AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
