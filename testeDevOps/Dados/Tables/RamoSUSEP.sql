CREATE TABLE [Dados].[RamoSUSEP] (
    [ID]     SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Codigo] SMALLINT      NULL,
    [Nome]   VARCHAR (100) NULL,
    [Grupo]  VARCHAR (70)  NULL,
    CONSTRAINT [PK_RAMOSUSEP] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_RamoSUSEP_ChangeTracking] on [Dados].[RamoSUSEP] for insert, update, delete
...
select @TableName = 'RamoSUSEP'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[RamoSUSEP]'
...
SELECT [ID],[Codigo],[Nome],[Grupo] INTO #ins FROM inserted

SELECT [ID],[Codigo],[Nome],[Grupo] INTO #del FROM deleted
