CREATE TABLE [Dados].[TipoMotivo] (
    [ID]     SMALLINT      IDENTITY (1, 1) NOT NULL,
    [Codigo] SMALLINT      NOT NULL,
    [Nome]   VARCHAR (100) NULL,
    CONSTRAINT [PK_TIPOMOTIVO] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [UNQ_TipoMotivo] UNIQUE NONCLUSTERED ([Codigo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_TipoMotivo_ChangeTracking] on Dados.TipoMotivo for insert, update, delete
...
select @TableName = 'TipoMotivo'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[TipoMotivo]'
...
CREATE TRIGGER [Dados].[trg_SendDBMail_TipoMotivo]
ON [Dados].[TipoMotivo]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.TipoMotivo, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Código ' + ISNULL(CAST(@Codigo AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
