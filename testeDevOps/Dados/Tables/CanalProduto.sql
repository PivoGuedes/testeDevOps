CREATE TABLE [Dados].[CanalProduto] (
    [IDCanal]   INT NOT NULL,
    [IDProduto] INT NOT NULL,
    CONSTRAINT [PK_CANALPRODUTO] PRIMARY KEY CLUSTERED ([IDCanal] ASC, [IDProduto] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_CANALPRO_FK_CANALP_CANALVEN] FOREIGN KEY ([IDCanal]) REFERENCES [Dados].[CanalVendaPAR] ([ID]),
    CONSTRAINT [FK_CANALPRO_FK_CANALP_PRODUTO] FOREIGN KEY ([IDProduto]) REFERENCES [Dados].[Produto] ([ID])
);


GO
CREATE NONCLUSTERED INDEX [idx_NCLCanalProduto_IDCanal]
    ON [Dados].[CanalProduto]([IDCanal] ASC)
    INCLUDE([IDProduto]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
    ON [PRIMARY];
...
CREATE TRIGGER [Dados].[Dados_CanalProduto_ChangeTracking] on [Dados].[CanalProduto] for insert, update, delete
...
select @TableName = 'CanalProduto'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[CanalProduto]'
...
SELECT [IDCanal],[IDProduto] INTO #ins FROM inserted

SELECT [IDCanal],[IDProduto] INTO #del FROM deleted
