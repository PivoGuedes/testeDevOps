CREATE TABLE [Dados].[Sexo] (
    [ID]        TINYINT      NOT NULL,
    [Descricao] VARCHAR (20) NOT NULL,
    [Sigla]     CHAR (1)     NULL,
    CONSTRAINT [PK_Sexo_ID] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);
...
CREATE TRIGGER [Dados].[Dados_Sexo_ChangeTracking] on [Dados].[Sexo] for insert, update, delete
...
select @TableName = 'Sexo'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[Sexo]'
...
SELECT [ID],[Descricao],[Sigla] INTO #ins FROM inserted

SELECT [ID],[Descricao],[Sigla] INTO #del FROM deleted
