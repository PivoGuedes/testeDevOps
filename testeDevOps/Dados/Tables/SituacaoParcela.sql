CREATE TABLE [Dados].[SituacaoParcela] (
    [ID]        TINYINT      NOT NULL,
    [Codigo]    TINYINT      NULL,
    [Descricao] VARCHAR (20) NULL,
    [TipoDado]  VARCHAR (30) NULL,
    CONSTRAINT [PK_SITUACAOPARCELA] PRIMARY KEY CLUSTERED ([ID] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
);


GO

CREATE TRIGGER [Dados].[trg_SendDBMail_SituacaoParcela]
ON [Dados].[SituacaoParcela]
AFTER INSERT
AS
...
;WITH Dados AS
(
SELECT 'Inserção dos Registros na Tabela - Dados.SituacaoParcela, ID do Registro: ' + ISNULL(CAST(@ID AS VARCHAR),'') + ', Código ' + ISNULL(CAST(@Codigo AS VARCHAR),'') + ', Nome do Registro: ' + ISNULL(@NomeRegistro,'') + '' AS Nome
)
...
CREATE TRIGGER [Dados].[Dados_SituacaoParcela_ChangeTracking] on [Dados].[SituacaoParcela] for insert, update, delete
...
select @TableName = 'SituacaoParcela'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[SituacaoParcela]'
...
SELECT [ID],[Codigo],[Descricao],TipoDado INTO #ins FROM inserted

SELECT [ID],[Codigo],[Descricao],TipoDado INTO #del FROM deleted
