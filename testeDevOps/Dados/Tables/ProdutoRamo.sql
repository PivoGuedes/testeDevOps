CREATE TABLE [Dados].[ProdutoRamo] (
    [IDProduto] INT      NOT NULL,
    [IDRamo]    SMALLINT NOT NULL,
    CONSTRAINT [PK_PRODUTORAMO] PRIMARY KEY CLUSTERED ([IDProduto] ASC, [IDRamo] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_PRODUTOR_FK_PRODUT_PRODUTO] FOREIGN KEY ([IDProduto]) REFERENCES [Dados].[Produto] ([ID]),
    CONSTRAINT [FK_PRODUTOR_FK_PRODUT_RAMO] FOREIGN KEY ([IDRamo]) REFERENCES [Dados].[Ramo] ([ID])
);
...
CREATE TRIGGER [Dados].[Dados_ProdutoRamo_ChangeTracking] on [Dados].[ProdutoRamo] for insert, update, delete
...
select @TableName = 'ProdutoRamo'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[ProdutoRamo]'
...
SELECT [IDProduto],[IDRamo] INTO #ins FROM inserted

SELECT [IDProduto],[IDRamo] INTO #del FROM deleted
