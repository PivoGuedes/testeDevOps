CREATE TABLE [Dados].[CanalMatricula] (
    [IDCanal]   INT          NOT NULL,
    [Matricula] VARCHAR (20) NOT NULL,
    CONSTRAINT [PK_CANALMATRICULA] PRIMARY KEY NONCLUSTERED ([IDCanal] ASC, [Matricula] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_CANALMAT_FK_CANALM_CANALVEN] FOREIGN KEY ([IDCanal]) REFERENCES [Dados].[CanalVendaPAR] ([ID])
);


GO
CREATE NONCLUSTERED INDEX [idx_NCLCanalMatricula_IDCanal]
    ON [Dados].[CanalMatricula]([IDCanal] ASC)
    INCLUDE([Matricula]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
    ON [PRIMARY];
...
CREATE TRIGGER [Dados].[Dados_CanalMatricula_ChangeTracking] on [Dados].[CanalMatricula] for insert, update, delete
...
select @TableName = 'CanalMatricula'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[CanalMatricula]'
...
SELECT [IDCanal],[Matricula] INTO #ins FROM inserted

SELECT [IDCanal],[Matricula] INTO #del FROM deleted
