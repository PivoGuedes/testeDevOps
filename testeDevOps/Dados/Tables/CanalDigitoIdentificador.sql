CREATE TABLE [Dados].[CanalDigitoIdentificador] (
    [IDCanal]    INT          NOT NULL,
    [Digito]     VARCHAR (20) NOT NULL,
    [Posicao]    TINYINT      NOT NULL,
    [DataInicio] DATE         NULL,
    [DataFim]    DATE         NULL,
    CONSTRAINT [PK_CANALDIGITO] PRIMARY KEY CLUSTERED ([IDCanal] ASC, [Digito] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE),
    CONSTRAINT [FK_CANALDIG_FK_CANALD_CANALVEN] FOREIGN KEY ([IDCanal]) REFERENCES [Dados].[CanalVendaPAR] ([ID])
);


GO
CREATE NONCLUSTERED INDEX [idx_NCLCanalDigitoIdentificador_IDCanal]
    ON [Dados].[CanalDigitoIdentificador]([IDCanal] ASC)
    INCLUDE([Digito], [Posicao]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE)
    ON [PRIMARY];
...
--        CanalDigitoIdentificador  

--                                        --

--------------------------------------------

CREATE TRIGGER [Dados].[Dados_CanalDigitoIdentificador_ChangeTracking] on [Dados].[CanalDigitoIdentificador] for insert, update, delete
...
select @TableName = 'CanalDigitoIdentificador'

select @TableSchema = 'Dados'

SELECT @TableName_Full = '[Dados].[CanalDigitoIdentificador]'
...
SELECT [IDCanal],[Digito],[Posicao],[DataInicio],[DataFim] INTO #ins FROM inserted

SELECT [IDCanal],[Digito],[Posicao],[DataInicio],[DataFim] INTO #del FROM deleted
