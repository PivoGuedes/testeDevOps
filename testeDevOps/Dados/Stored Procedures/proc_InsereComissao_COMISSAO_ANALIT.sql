...
/*******************************************************************************
	Nome: CORPORATIVO.Dados.proc_InsereComissao_COMISSAO_ANALIT
...
CREATE PROCEDURE [Dados].[proc_InsereComissao_COMISSAO_ANALIT](@DataComissao DATE = NULL) 
AS
...
BEGIN
  ...
  SET @COMANDO = 'SELECT @MaiorCodigo= EM.MaiorData
                ...
                '' SELECT MAX(DataArquivo) [MaiorData]
                   FROM FENAE.dbo.COMISSAO_ANALIT
...
WHILE @PontoDeParada <= @MaiorCodigo AND @PontoDeParada < '2016-01-01'
BEGIN 
...
SELECT 	
	...
	ValorCorretagemAgrupado     ,
	DataQuitacaoParcela           
FROM  OPENQUERY ([OBERON], ''EXEC [Fenae].[Corporativo].[proc_RecuperaComissaoAnaliticoDetalhado] ''''' + Cast(@PontoDeParada as varchar(20)) + ''''''') CAD'
...
/**************************************/
/*;INSERT INTO CAD_TEMP (CodigoFilial, CodigoFilialProposta, CodigoOperacao, CodigoPontoVenda, CodigoProdutor,  CodigoSubGrupoRamoVida,
                   DataArquivo, DataCalculo, DataRecibo, NumeroApolice, NumeroEndosso, NumeroParcela, NumeroProposta, NumeroCertificado,
                   PercentualCorretagem, TipoCorretagem, ValorBase, ValorCorretagem, ValorBaseAgrupado, ValorCorretagemAgrupado,
                   numerorecibo, CodigoProduto, NomeCliente)
Select CodigoFilial, CodigoFilialProposta, 9999 [CodigoOperacao], 630 [CodigoPontoVenda], CodigoProdutor,  null [CodigoSubGrupoRamoVida],
       DataArquivo, GETDATE() [DataCalculo], DataRecibo, NumeroApolice, NumeroEndosso, null [NumeroParcela], null [NumeroProposta], null [NumeroCertificado],
       null [PercentualCorretagem], null [TipoCorretagem], 
        ValorBaseAgrupado - ValorBase  [ValorBase-DiferencaValorBase],
        ValorCorretagemAgrupado - ValorCorretagem [ValorCorretagem-DiferencaValorCorretagem],         
       ValorBaseAgrupado, ValorCorretagemAgrupado,
       numerorecibo, CodigoProduto, null [NomeCliente]--, ValorBase, ValorCorretagem,  
from
(
  select SUM(valorBase * co.SinalOperacao) valorBase, SUM(ValorCorretagem * co.SinalOperacao)  ValorCorretagem
          , (ValorBaseAgrupado * co.SinalOperacao ) ValorBaseAgrupado, (ValorCorretagemAgrupado * co.SinalOperacao) ValorCorretagemAgrupado
          , CodigoProduto,  CodigoOperacao, NumeroApolice, NumeroEndosso , codigofilialproposta, numerorecibo, codigoprodutor, dataarquivo, CODIGOFILIAL, DataRecibo , CodigoRamo
  FROM CAD_TEMP CAD
  INNER JOIN Dados.ComissaoOperacao CO
  ON CO.Codigo = CAD.CodigoOperacao 
  where NumeroEndosso = 219981
  GROUP BY CodigoProduto,  CodigoOperacao, NumeroApolice, NumeroEndosso, CO.SinalOperacao, ValorBaseAgrupado, ValorCorretagemAgrupado, codigofilialproposta, numerorecibo, codigoprodutor, dataarquivo, CODIGOFILIAL, DataRecibo, CodigoRamo

) Diferenca
where Diferenca.valorBase <> Diferenca.valorBaseAgrupado or Diferenca.ValorCorretagem <> Diferenca.ValorCorretagemAgrupado*/
...
FROM       
(
SELECT SUM(Cast(valorBase AS decimal(20,6))) * CO.SinalOperacao [ValorBase], ValorBaseAgrupado * CO.SinalOperacao [ValorBaseAgrupado]
     , SUM(Cast(ValorCorretagem AS decimal(20,6))) * CO.SinalOperacao [ValorCorretagem], ValorCorretagemAgrupado * CO.SinalOperacao [ValorCorretagemAgrupado]
     , CodigoProduto       , NumeroApolice, NumeroEndosso, CodigoOperacao
     , codigofilialproposta, numerorecibo, codigoprodutor
     , dataarquivo, CODIGOFILIAL, DataRecibo        
FROM CAD_TEMP  A
  INNER JOIN Dados.ComissaoOperacao CO
  ON CO.Codigo = A.CodigoOperacao 
WHERE ValorBase <> ValorBaseAgrupado or ValorCorretagem <> ValorCorretagemAgrupado                                    
GROUP BY ValorBaseAgrupado, ValorCorretagemAgrupado, CodigoProduto,  CodigoOperacao, NumeroApolice, NumeroEndosso
       , codigofilialproposta, numerorecibo, codigoprodutor, dataarquivo, CODIGOFILIAL, DataRecibo, CO.SinalOperacao                                     
HAVING SUM(Cast(valorBase AS decimal(20,6))) <> ValorBaseAgrupado  or SUM(Cast(ValorCorretagem AS decimal(20,6))) <> ValorCorretagemAgrupado 
) A
/*******************************************************************************************************************************************************************************************************************************************/
...
print '3'                  
		...
		;MERGE INTO Dados.Proposta AS T
	      USING (SELECT DISTINCT EM.[NumeroApolice], EM.[NumeroPropostaTratado], 1 [IDSeguradora], PRD.ID [IDProduto], 'FN0701B.COMISSAO' [NomeArquivo], @PontoDeParada [DataArquivo]
				FROM CAD_TEMP EM
...
;MERGE INTO Dados.PropostaCliente AS T
	      USING (SELECT DISTINCT PRP.ID [IDProposta], CASE WHEN NomeCliente IS NULL OR NomeCliente = '' THEN 'CLIENTE DESCONHECIDO - PROPOSTA NÃO RECEBIDA' ELSE [NomeCliente] END [NomeCliente], 'FN0701B.COMISSAO' [NomeArquivo], @PontoDeParada [DataArquivo]
            FROM CAD_TEMP EM
...
/*INSERE PROPOSTAS NÃO LOCALIZADAS - POR NÚMERO DE CERTIFICADO*/
		;MERGE INTO Dados.Proposta AS T
	      USING (SELECT DISTINCT EM.[NumeroApolice], EM.[NumeroCertificadoTratado], 1 [IDSeguradora], PRD.ID [IDProduto], 'FN0701B.COMISSAO' [NomeArquivo], @PontoDeParada [DataArquivo]
				 FROM CAD_TEMP EM
				 	 INNER JOIN Dados.Produto PRD
					 ON PRD.CodigoComercializado = EM.CodigoProduto
       ...
       WHEN NOT MATCHED
		          THEN INSERT (NumeroProposta,              [IDSeguradora],    IDAgenciaVenda,    IDProduto,   IDProdutoSIGPF, IDCanalVendaPAR, IDSituacaoProposta, IDTipoMotivo, TipoDado, DataArquivo)
		               VALUES (X.[NumeroCertificadoTratado], X.[IDSeguradora],                -1,  X.IDProduto,                0,              -1,                  0,           -1, 'COMISSAO', X.DataArquivo)	
	   WHEN MATCHED
	     THEN UPDATE
		  /*Atualiza o código do Produto & código do Produto Anterior com o código recebido no comissionamento
		    Egler - 23/09/2013*/
    ...
    ;MERGE INTO Dados.ContratoCliente AS T
		USING (
            SELECT DISTINCT
                   ...
                   , C.[DataArquivo],'FN0701B.COMISSAO' [Arquivo]
...
USING (
        SELECT *
            ...
            SELECT 
                 ...
                 , /*@PontoDeParada*/ CAD.[DataArquivo], 'FN0701B.COMISSAO' [Arquivo]
...
SELECT R.ID [IDRamo], CAD.PercentualCorretagem, CAD.ValorCorretagem * CO.SinalOperacao,
       ...
       PRD.ID [IDProduto], U.ID [IDUnidadeVenda], COALESCE(PRP.ID, CRT.IDProposta) [IDProposta], 'FN0701B' [Arquivo],  CAD.DataArquivo
...
/*CALCULA OS CANAIS DE VENDA DA DATA*/
   BEGIN TRY     
	   /*CALCULA OS CANAIS DE VENDA DA DATA*/
	   EXEC Dados.proc_AtualizaCanalVendaPAR_Comissao @DataArquivo = @PontoDeParada, @ForcarAtualizacaoCompleta = 0;
...
OPENQUERY ([OBERON], ''SELECT *
                      FROM FENAE.dbo.RECOMISS_FILIAL RF 
...
print '18'  
  ...
  ;MERGE INTO ControleDados.ExportacaoFaturamento AS T
	  USING (
        SELECT DISTINCT DataRecibo, NumeroRecibo, DataRecibo [DataCompetencia], 3 IDEmpresa
...
print '21'  
 ...
 print '22'  
  ...
  /*************************************************************************************/
  UPDATE Dados.Comissao SET IDProposta = CRT.IDProposta
...
--EXEC [Dados].[proc_InsereComissao_COMISSAL_ANALIT] '2014-02-19'
