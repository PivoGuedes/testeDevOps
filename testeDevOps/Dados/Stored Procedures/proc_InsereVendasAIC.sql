...
/*
	...
	Última alteração : 23/11 Adaptação a novo layout de arquivo by Sandrine 

*/

/*******************************************************************************
	Nome: CORPORATIVO.Dados.proc_InsereVendasAIC
...
CREATE PROCEDURE [Dados].[proc_InsereVendasAIC] as
BEGIN TRY		
...
WHERE NomeEntidade = 'VENDASAIC_Desep'
...
SET @COMANDO = 'INSERT INTO [dbo].[VENDASAIC_TEMP] 
                ...
                ''EXEC [DESEP].[Corporativo].[proc_RecuperaVendas_AIC] ''''' + @PontoDeParada + ''''''') PRP'
...
*/
	--select * from Dados.PropostaAIC  order by id desc
	-----------------------------------------------------------------------------------------------------------------------
		...
		USING (
		       ...
		       (
					SELECT 
						...
						,COD_TP_RENOVACAO  as TipoSeguro
					FROM dbo.[VENDASAIC_TEMP] VAC
