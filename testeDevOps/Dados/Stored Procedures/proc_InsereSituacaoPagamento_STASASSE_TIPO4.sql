...
create PROCEDURE [Dados].[proc_InsereSituacaoPagamento_STASASSE_TIPO4] 
AS
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP]') AND type in (N'U') /*ORDER BY NAME*/)
	DROP TABLE [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP];

CREATE TABLE [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP]
(
	[Codigo] [bigint] NOT NULL,
	[NumeroProposta] [varchar](20) NOT NULL,
	[SituacaoCobranca] [char](3) NULL,
	[DataInicioSituacao] [date] NULL,
	[QtdeParcelasPagas] [smallint] NULL,
	[QtdeTotalParcelas] [smallint] NULL,	
 ...
 /*Cria Índices*/  
CREATE CLUSTERED INDEX idx_SituacaoPagamento_STASASSE_TEMP ON [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] (Codigo ASC)  

CREATE NONCLUSTERED INDEX IDX_NumeroProposta_STASASSE_TEMP  ON [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] ([NumeroProposta],[SituacaoCobranca],[DataInicioSituacao], [QtdeParcelasPagas], [DataArquivo] DESC, Codigo DESC)

SELECT @PontoDeParada = PontoParada
FROM ControleDados.PontoParada
WHERE NomeEntidade = 'SituacaoPagamento_STASASSE_Tipo4'
...
SET @COMANDO = 'INSERT INTO [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] 
				(      
					[Codigo]
					,[NumeroProposta]
					,[SituacaoCobranca]
                ...
                ''EXEC [Fenae].[Corporativo].[proc_RecuperaSituacaoPagamento_STASASSE_TIPO4] ''''' + @PontoDeParada + ''''''') PRP'

EXEC (@COMANDO)     

SELECT @MaiorCodigo = MAX(PRP.Codigo)
FROM [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] PRP
...
(
	SELECT distinct PGTO.NumeroProposta, 
		   1 [IDSeguradora], -- Aruumar PGTO.[IDSeguradora]
		   MIN(PGTO.NomeArquivo) [TipoDado], 
		   MIN(PGTO.DataArquivo) DataArquivo
    FROM [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] PGTO
...
WHEN NOT MATCHED THEN 
 ...
 UPDATE	 [SituacaoPagamento_STASASSE_TIPO4_TEMP]  SET IDProposta = PRP.IDProposta, IDSituacaoCobranca = SC.IDSituacaoCobranca
 FROM [SituacaoPagamento_STASASSE_TIPO4_TEMP] TEMP
 ...
 UPDATE Dados.[PropostaSituacaoPagamento] SET LastValue = 0
   -- SELECT *
    ...
    WHERE PS.IDProposta IN (
	                        SELECT t.IDProposta
                            FROM  [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] t
		...
		( SELECT * 
					FROM (
							...
							SELECT DISTINCT [Codigo]
								  ...
								  ,[SituacaoCobranca]
								  ,[DataInicioSituacao]
								  ,[QtdeParcelasPagas] AS ParcelasPagas
								  ,[QtdeTotalParcelas] AS TotalDeParcelas
								  ,[NomeArquivo]
								  ,temp.[TipoDado]
								  ,temp.[DataArquivo]
								--  ,[ControleVersao]
								  ,ROW_NUMBER() OVER (PARTITION BY temp.[NumeroProposta],[SituacaoCobranca],[DataInicioSituacao], [QtdeParcelasPagas] ORDER BY temp.[DataArquivo] DESC, Codigo DESC) [ORDER]   

							  FROM [dbo].[SituacaoPagamento_STASASSE_TIPO4_TEMP] TEMP
...
TRUNCATE TABLE [dbo].SituacaoPagamento_STASASSE_TIPO4_TEMP
