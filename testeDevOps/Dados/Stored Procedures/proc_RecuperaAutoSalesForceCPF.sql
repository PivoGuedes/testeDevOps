CREATE PROCEDURE [Dados].[proc_RecuperaAutoSalesForceCPF] 	@cpf VARCHAR(20)
...
;WITH autom AS (
SELECT  
p.NumeroProposta, p.DataProposta,coalesce(pa.DataInicioVigencia, p.DataInicioVigencia) DataInicioVigencia,COALESCE(pa.DataFimVigencia,c.DataFimVigencia,p.DataFimVigencia)  AS DataFimVigencia,
		coalesce(c.NumeroContrato,p.NumeroProposta) NumeroContrato,v.Nome + ' ' +pa.Placa Identificacao, c.ValorPremioTotal,pd.CodigoComercializado,p.ValorPremioBrutoEmissao,
...
--LEFT JOIN dados.contrato c2 ON P.IDContrato = c.ID
LEFT JOIN dados.PropostaAutomovel pa 
ON pa.IDProposta = p.ID and pa.LastValue = 1
...
SELECT a.NumeroProposta,a.DataProposta,a.DataInicioVigencia,a.DataFimVigencia,
	...
	ISNULL(pg.ValorPrimeiraParcela,pg2.ValorDemaisParcelas) AS ValorPrimeiraParcela,pg2.ValorDemaisParcelas,a.idproposta,a.DataEmissao,'AUTO CS' as TipoDeProduto
	--into #autotemp
 FROM autom a
