CREATE PROCEDURE [Dados].[proc_RecuperaMR_RD_SalesForceCPF] 	@cpf VARCHAR(14)
...
p.NumeroProposta, p.DataProposta,p.DataInicioVigencia,COALESCE(c.DataFimVigencia,p.DataFimVigencia) AS DataFimVigencia,
		...
		pd.Descricao,coalesce(p.IDProduto,e.idproduto) IDproduto,c.ID,p.id AS idproposta,c.QuantidadeParcelas,c.DataEmissao,pen.Endereco + ' ' + pen.bairro + ' ' + pen.Cidade + ' ' + pen.UF AS Identificacao--,c.ValorPremioTotal
...
SELECT a.NumeroProposta,a.DataProposta,a.DataInicioVigencia,a.DataFimVigencia,
	...
	ISNULL(pg.ValorPrimeiraParcela,pg2.ValorDemaisParcelas) AS ValorPrimeiraParcela,pg2.ValorDemaisParcelas,a.idproposta,a.DataEmissao,a.Identificacao,'MR/RD' AS TipoDeProduto
	--INTO #mrtemp
 FROM autom a
