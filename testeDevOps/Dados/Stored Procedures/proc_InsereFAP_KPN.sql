...
/*******************************************************************************
Nome: Corporativo.Dados.proc_InsereFAP_KPN	
--*******************************************************************************/
CREATE PROCEDURE [Dados].[proc_InsereFAP_KPN] 
AS
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FAP_KPN_TEMP]') AND type in (N'U') /*ORDER BY NAME*/)
	DROP TABLE [dbo].[FAP_KPN_TEMP];

CREATE TABLE [dbo].[FAP_KPN_TEMP]
(
 ...
 /*Cria Índices */  
CREATE CLUSTERED INDEX idx_FAP_KPN_TEMP 
ON [dbo].FAP_KPN_TEMP
( 
  ID ASC
)   

CREATE NONCLUSTERED INDEX idx_NDX_NumeroProposta_FAP_KPN_TEMP
ON [dbo].FAP_KPN_TEMP
( 
  CPF ASC
)       
...
SELECT 	
	...
	Cod_Mailing,
	Produto_Efetivado
FROM OPENQUERY ([OBERON], ''EXEC [Fenae].[Corporativo].[proc_RecuperaFAP_KPN] ''''' + @PontoDeParada + ''''''') PRP'	   

EXEC (@COMANDO)  
		   
SELECT @MaiorCodigo = MAX(ID)
FROM [dbo].[FAP_KPN_TEMP]
...
TRUNCATE TABLE [dbo].[FAP_KPN_TEMP]
