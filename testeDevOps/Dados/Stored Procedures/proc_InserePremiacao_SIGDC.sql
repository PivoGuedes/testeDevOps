...
EXEC [Dados].[proc_InserePremiacao_SIGDC] @TipoAcordo='I'

*******************************************************************************/
CREATE PROCEDURE [Dados].[proc_InserePremiacao_SIGDC] @TipoAcordo VARCHAR(2)
AS
...
DECLARE @PontoDeParada AS DATE	= '2016-05-05'
DECLARE @COMANDO AS NVARCHAR(max) 
DECLARE @MaiorCodigo AS DATE

IF  @TipoAcordo ='GI'  
	BEGIN
	  SET @NomeEntidade = 'Premiacao_GERENTE_INDICADORES' ;
	...
	ELSE
	  IF @TipoAcordo = 'I' 
	  BEGIN
		SET @NomeEntidade = 'Premiacao_INDICADORES'
	  ...
	  ELSE
		IF	 @TipoAcordo = 'SR' 
		BEGIN
		  SET @NomeEntidade = 'Premiacao_SUPERINTENDENTE_INDICADORES' ;
...
SET @COMANDO = 'SELECT @MaiorCodigo= EM.MaiorData
            ...
            '' SELECT MAX(DataArquivo) [MaiorData]
                FROM FENAE.dbo.PremiacaoAnaliticoIndicadores_SIGDC
...
--PRINT @PontoDeParada
    ...
    EXEC [Dados].[proc_InserePremiacaoAnalitico_INDICADORES_SIGDC] @PontoDeParada, @TipoAcordo;

    EXEC [Dados].[proc_InserePremiacaoSintetico_INDICADORES_SIGDC] @PontoDeParada, @TipoAcordo;
...
--select sum(valorbruto) from Dados.RePremiacaoIndicadores where idlote=140
--select sum(valorbruto) from Dados.RePremiacaoIndicadores where dataarquivo='2016-02-05' and gerente=0
--commit
