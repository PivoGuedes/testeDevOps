...
*******************************************************************************/
CREATE PROCEDURE [Dados].[proc_InserePremiacao_CONSCBN] 
AS
...
CREATE TABLE [dbo].[PREMIACAO_CONSCBN_TEMP](
	...
	[CodigoProdutor] [int] NOT NULL DEFAULT(2), --Codigo 2 ->FPC PAR CORRETORA DE SEGUROS SA - PREMIACAO CONSÓRCIO CBN
...
WHERE NomeEntidade = 'PREMIACAO_CONSCBN'
...
/*********************************************************************************************************************/
   SET @COMANDO =
    '  INSERT INTO dbo.PREMIACAO_CONSCBN_TEMP
       ...
       ''EXEC [Fenae].[Corporativo].[proc_recuperaPremiacao_CONSCBN]''''' + @PontoDeParada + ''''''') PRP
         '
exec (@COMANDO)    
