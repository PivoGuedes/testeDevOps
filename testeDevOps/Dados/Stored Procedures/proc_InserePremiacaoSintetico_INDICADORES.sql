...
/*
	...
	William Moreira -> 2018-01-09 - No MERGE na tabela [Dados].[RePremiacaoIndicadores] foi adicionada verificação pelo campo NomeArquivo como critério de inserção.
...
CREATE PROCEDURE [Dados].[proc_InserePremiacaoSintetico_INDICADORES] (@PontoDeParada AS DATE, @TipoAcordo VARCHAR(2) = 'I') as
BEGIN TRY	

 
--DECLARE @TipoAcordo VARCHAR(2) = 'SR'
--DECLARE @PontoDeParada AS date = '2015-08-05'
...
--IF (@TipoAcordo = 'GI')   
--BEGIN
--  SET @NomeEntidade = 'PremiacaoSintetico_GERENTE_INDICADORES_Odonto' 
...
--	[TipoDado] AS (Cast(CASE WHEN CHARINDEX('CO3381B', NomeArquivo) > 1 THEN 'CO3381B' ELSE 'CO318B' END AS VARCHAR(30))) PERSISTED,
	[TipoDado] AS (Cast(CASE WHEN CHARINDEX('ODONTO', NomeArquivo) > 1 THEN 'ODONTO' ELSE 'CO318B' END AS VARCHAR(30))) PERSISTED,
...
--select @PontoDeParada = 20007037
	...
	SET @COMANDO = 'INSERT INTO [dbo].[PremiacaoSintetico_INDICADORES_TEMP] (
				  ...
				  ''SET NOCOUNT ON;
                    set fmtonly off; EXEC [Fenae].[Corporativo].proc_RecuperaPremiacaoSintetico_INDICADORES ''''' + Cast(@COMANDO  as varchar(14)) + ''''''') PRM'
	...
	***********************************************************************/
	...
	USING (
		...
		SELECT F.ID [IDFuncionario], UA.ID [IDUnidade], PRM.[CPF], PRM.[Nome] [NomeIndicador], '104' Banco
		     , ISNULL([NumeroConta] + '-' + ISNULL([DigitoConta],''),0) [ContaCorrente], ISNULL(PRM.[OperacaoConta],0) AS OperacaoConta
	...
	AND X.[NomeArquivo] = T.[NomeArquivo]
	WHEN NOT MATCHED
