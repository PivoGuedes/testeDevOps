...
/*
	...
	Última alteração: 15/12/2017 
	Autor: Danilo Gama
	Descrição: Alteração na regra de envio dos registros ao box. Não deixa enviar registros sem CPF/CNPJ, Nome e Matrícula.
...
*******************************************************************************/
CREATE PROCEDURE [Dados].[proc_InserePremiacao_SAFCBN_SIGDC] 
AS
					...
					FROM dbo.PREMIACAO_SAFCBN_TEMP PST
						...
						OUTER APPLY (						            
						             ...
						             WHERE CRP.Matricula = PST.NumeroMatricula
									   ...
									   AND ((CRP.CPFCNPJ is not null and ltrim(rtrim(CRP.CPFCNPJ)) <> '') and (CRP.Nome is not null and ltrim(rtrim(CRP.Nome)) <> '') and (CRP.Matricula is not null and ltrim(rtrim(CRP.Matricula)) <> '0'))
