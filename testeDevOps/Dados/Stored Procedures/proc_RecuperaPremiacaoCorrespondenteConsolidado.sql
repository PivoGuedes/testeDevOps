...
*******************************************************************************/
CREATE PROCEDURE [Dados].[proc_RecuperaPremiacaoCorrespondenteConsolidado] (@Ano smallint, @Mes tinyint, @IDTipoCorrespondente tinyint, @IDTipoProduto tinyint)
AS
...
AND PC.IDTipoProduto= @IDTipoProduto
...
--AND CASE WHEN @IDtipoCorrespondente = 1 AND PC.Arquivo LIKE '%SAFAT%' THEN 0
--        WHEN @IDtipoCorrespondente = 2 AND PC.Arquivo LIKE '%SACBN%' THEN 0
--AND CASE WHEN @IDtipoCorrespondente = 1 AND (PC.Arquivo LIKE '%SAFAT%' OR PC.Arquivo LIKE '%DC0701_CBN_ATENDENTE%' )THEN 0
--        WHEN @IDtipoCorrespondente = 2  AND (PC.Arquivo LIKE '%SACBN%' OR PC.Arquivo LIKE '%DC0701_CBN_EMPRESARIO%') THEN 0

--            ELSE 1
--    END = 1      

--and Arquivo LIKE '%DC0701_CBN_EMPRESARIO%'
--------------------------------------------------------------------------------------
GROUP BY 
       PC.[IDProdutor], 
       PC.[IDCorrespondente],
