...
/*******************************************************************************
	Nome: CORPORATIVO.Dados.proc_InsereW713AMET
	Descrição: Procedimento que realiza a carga do AVGESTAO (W713) no banco de dados.
...
*******************************************************************************/
CREATE PROCEDURE [Dados].[proc_InsereW713AMET] 
AS
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[W713AMET_TEMP]') AND type in (N'U') /*ORDER BY NAME*/)
BEGIN
	DROP TABLE [dbo].[W713AMET_TEMP];
	/*
		RAISERROR   (N'A tabela temporária [W713AMET_TEMP] foi encontrada. Verifique se há um processo paralelo executando a função.',
    16, -- Severity.
    1 -- State.
    );
    */
...
CREATE TABLE [dbo].[W713AMET_TEMP](
	[Codigo] [int]  NOT NULL,
...
CREATE CLUSTERED INDEX idx_W713AMET_TEMP_Codigo ON [dbo].[W713AMET_TEMP]
( 
  Codigo ASC
)   
...
/*********************************************************************************************************************/
    ...
    '  INSERT INTO dbo.W713AMET_TEMP
     ...
     SELECT 
       ...
       ''EXEC [Fenae].[Corporativo].[proc_RecuperaW713AMET] ''''' + @PontoDeParada + ''''''') PRP
    '
exec (@COMANDO)    

SELECT @MaiorCodigo= MAX(PRP.Codigo)
FROM dbo.W713AMET_TEMP PRP                    
...
--print @MaiorCodigo
  ...
  SELECT DISTINCT U.ID, 'UNIDADE COM DADOS INCOMPLETOS' [Unidade], -1 [CodigoNaFonte], 'W713' [TipoDado], MAX(W.DataArquivo) [DataArquivo], 'W713' [Arquivo]
  FROM [dbo].[W713AMET_TEMP] W
  ...
  /*#################################################################*/
    ...
    /*Remove a marcação LastValue dos registros recebidas para atualizar a última posição*/
    UPDATE Dados.AVGestaoBloco SET LastValue = 0
   -- SELECT *
    FROM Dados.AVGestaoBloco PS
    WHERE EXISTS (SELECT *
	              FROM [dbo].[W713AMET_TEMP] W
		             INNER JOIN Dados.Unidade U
		             ON W.CodigoUnidade = U.Codigo
		             ...
		             ON B.Codigo = W.Bloco
				   WHERE PS.IDUnidade = U.ID
				     AND PS.IDBloco = B.ID
					 AND PS.IDSegmento = S.ID
					 AND PS.AnoPeriodo = W.AnoPeriodo
					 AND PS.MesPeriodo = W.MesPeriodo
                  )
           AND PS.LastValue = 1
  ...
  /*************************************************************************************/ 
  	...
  	/*Atualiza a marcação LastValue dos registros recebidos para atualizar a última posição*/
    UPDATE Dados.AVGestaoBloco SET LastValue = 1
    FROM Dados.AVGestaoBloco PE
	INNER JOIN (
				SELECT ID,  ROW_NUMBER() OVER(PARTITION BY  IDUnidade, IDBloco, IDSegmento, AnoPeriodo, MesPeriodo
	                                          ORDER BY  IDUnidade ASC, IDBloco ASC, IDSegmento ASC, AnoPeriodo ASC
											          , MesPeriodo ASC, DataArquivo DESC, ISNULL(TRY_PARSE(RIGHT(SUBSTRING(ARQUIVO, CHARINDEX('REPRO',ARQUIVO, 1),7),2) as int),0) DESC) [ORDEM]
				FROM Dados.AVGestaoBloco PS
			    WHERE EXISTS (SELECT *
							  FROM [dbo].[W713AMET_TEMP] W
								 INNER JOIN Dados.Unidade U
  ...
  /*********************************************************************************************************************/
  TRUNCATE TABLE [dbo].[W713AMET_TEMP]        
  /*********************************************************************************************************************/
...
--EXEC [Dados].[proc_InsereW713AMET] 
