...
/*******************************************************************************
	Nome: CORPORATIVO.Dados.proc_InsereBeneficiario_PRPFPREV
	Descrição: Procedimento que realiza a inserção dos BENEFICIARIOS (PRPFPREV) no banco de dados.
...
CREATE PROCEDURE [Dados].[proc_InsereBeneficiario_PRPFPREV] as
BEGIN TRY		
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Beneficiario_PRPFPREV_TEMP]') AND type in (N'U') /*ORDER BY NAME*/)
	DROP TABLE [dbo].[Beneficiario_PRPFPREV_TEMP];

CREATE TABLE [dbo].[Beneficiario_PRPFPREV_TEMP](
...
CREATE CLUSTERED INDEX idx_Beneficiario_PRPFPREV_TEMP ON [dbo].[Beneficiario_PRPFPREV_TEMP]
( 
  Codigo ASC
)         
...
WHERE NomeEntidade = 'Beneficiario_PRPFPREV'
...
SET @COMANDO = 'INSERT INTO [dbo].[Beneficiario_PRPFPREV_TEMP] (
                      [Codigo],                                                 
	                    [ControleVersao],                                         
	                    [NomeArquivo],                                            
	                    [DataArquivo],        
                ...
                ''EXEC [Fenae].[Corporativo].[proc_RecuperaBeneficiario_PRPFPREV] ''''' + @PontoDeParada + ''''''') PRP'

EXEC (@COMANDO)     

                
SELECT @MaiorCodigo= MAX(PRP.Codigo)
FROM [dbo].[Beneficiario_PRPFPREV_TEMP] PRP
...
--    PRINT @MaiorCodigo
      ...
      ;MERGE INTO Dados.Proposta AS T
	      USING (SELECT  EM.[NumeroPropostaTratado], 4 [IDSeguradora]
	                           , MAX(TipoArquivo) [TipoDado], MAX(DataArquivo) [DataArquivo]
               FROM [Beneficiario_PRPFPREV_TEMP] EM
      ...
      ;MERGE INTO Dados.PropostaCliente AS T
	      USING (SELECT  PRP.ID [IDProposta], 4 [IDSeguradora], 'CLIENTE DESCONHECIDO - PROPOSTA NÃO RECEBIDA' [NomeCliente], MAX(TipoArquivo) [TipoDado], MAX(EM.[DataArquivo]) [DataArquivo]
            FROM [Beneficiario_PRPFPREV_TEMP] EM
   ...
   ***********************************************************************/
   ...
   /***********************************************************************/   
    -- begin tran             		          
    -----------------------------------------------------------------------------------------------------------------------
    --Comando para inserir os Beneficiários dos certificados recebidos no arquivo PRPFPREV infoVariavel TP4
    -----------------------------------------------------------------------------------------------------------------------		               
   	MERGE INTO Dados.PropostaBeneficiario AS T
		USING (             
              ...
              FROM [Beneficiario_PRPFPREV_TEMP] BEN
 ...
 /*********************************************************************************************************************/
  TRUNCATE TABLE [dbo].[Beneficiario_PRPFPREV_TEMP] 
