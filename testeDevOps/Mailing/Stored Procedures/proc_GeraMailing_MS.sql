...
CREATE PROCEDURE [Mailing].[proc_GeraMailing_MS](@DataMailing DATE /*A data será subtraida de um para a correta referência do mailing*/ )
AS
...
BEGIN 
	...
	;  WITH Temp AS (
		SELECT DISTINCT CPF_CNPJ_CLIENTE COLLATE Latin1_General_CI_AS AS CPF FROM OBERON.[COL_MULTISEGURADORA].[dbo].[Tabela_Mailing_Diario_MS] WHERE Codigo_campanha<>'CGAUTCOL20151215' AND Data_inclusao_calculo> DATEADD(DD,-60,@DataMailingRegistro) --and Data_inclusao_calculo < '2015-11-04'
		UNION
		SELECT DISTINCT CPF FROM Mailing.MailingAutoMS WHERE DataRefMailing>=DATEADD(DD, -60 -@SomaDiaFim, @DataMailingRegistro) --and DataRefMailing < '2015-11-04'
		UNION
		SELECT DISTINCT  SUBSTRING(CPF,1,3) + '.' + SUBSTRING(CPF,4,3) + '.' + SUBSTRING(CPF,7,3) + '-' + SUBSTRING(CPF,10,2) FROM Mailing.MailingParIndica 
		UNION
		SELECT DISTINCT SUBSTRING(CPF,1,3) + '.' + SUBSTRING(CPF,4,3) + '.' + SUBSTRING(CPF,7,3) + '-' + SUBSTRING(CPF,10,2) COLLATE Latin1_General_CI_AS FROM Mailing.BlackList
	...
	AND ISNULL(TV.TipoVeiculo,'') NOT IN ('Moto', 'Caminhão') --Incluído filtro caminhão à pedido da Bianca, do time DBM_MKT
	AND NOT EXISTS (
	                SELECT *
	...
	DECLARE @NaoCotados VARCHAR(10) = (SELECT ISNULL(COUNT(*),0) AS NCotados FROM mailing.mailingautoms WHERE datarefmailing=@DataMailingRegistro AND DataEnvioMailing IS NULL)
	...
	WITH C AS (
		SELECT ISNULL(RegraAplicada,'Não identificado') AS RegraAplicada, count(*) AS QtdLeads FROM mailing.mailingautoms WHERE datarefmailing=@DataMailingRegistro GROUP BY RegraAplicada
	)
	SELECT * FROM C;
