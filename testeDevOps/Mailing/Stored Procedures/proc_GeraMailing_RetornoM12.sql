...
CREATE PROCEDURE [Mailing].[proc_GeraMailing_RetornoM12](@DataMailing DATE /*A data será subtraida de um para a correta referência do mailing*/ )
AS
...
-------------------------------------------------------------------------------------------------------------------------------------------
------Carrega tabela temporária Mundo Caixa --INÍCIO		
-------------------------------------------------------------------------------------------------------------------------------------------		
	
SELECT * INTO #MUNDOCAIXA
FROM DBM.DBM_MKT.DBO.MUNDO_CAIXA

CREATE CLUSTERED INDEX [IDX_CL_MUNDOCAIXA] ON #MUNDOCAIXA ([CPFCNPJ] ASC)

CREATE NONCLUSTERED INDEX [IDX_NCL_MUNDOCAIXA_CPF] ON #MUNDOCAIXA ([CPFCNPJ] ASC)
INCLUDE ([Nome],[DataNascimento],[EmailPessoal],[DDDResidencial],[TelefoneResidencial],[Matricula]) 
WHERE ([CPFCNPJ]<>'000')
	
-------------------------------------------------------------------------------------------------------------------------------------------
------Carrega tabela temporária Mundo Caixa --FIM		
-------------------------------------------------------------------------------------------------------------------------------------------		


IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ClientesM12_TEMP]') AND type in (N'U') /*ORDER BY NAME*/)
	DROP TABLE [dbo].[ClientesM12_TEMP];

	
CREATE TABLE [dbo].[ClientesM12_TEMP]
(
[ID] [int] NOT NULL,
...
WHERE Arquivo like '%DEVOLUÇÃO_PAR_%_CAIXA SEGUROS%'
------(
...
(
	(
		...
		AND NOT EXISTS --Sem negócio fechado
								
						(
							SELECT *
							FROM Dados.Atendimento B WITH (NOLOCK)
							WHERE B.NomeCampanha = 'AQAUTSIA20160301' ----Lincoln(20160316): incluido o filtro codigo de campanha
	...
	OR --OU QUE NO DIA ANTERIOR TENHA DECLINADO (Recursa)
	(
...
/********************************************************************************************/
UPDATE [ClientesM12_TEMP] 
SET RegraAplicada='Sem negócio fechado D5' 
WHERE IDStatusLigacao in 
			(
				SELECT ID
...
UPDATE [ClientesM12_TEMP] 
SET RegraAplicada='Recusa dia anterior' 
WHERE IDStatusLigacao in 
			(
			SELECT ID
			FROM Dados.Status_Ligacao
...
'AQUISIÇÃO AUTO - RETORNO M12_MS' [NOME_CAMPANHA],  --'AQUISICAO_SEGURO AUTO_MS'
'AQAUTOMSAA20160301' [CODIGO_CAMPANHA],
'AQAUTOMSAA20160301_' + REPLACE(Convert(date,@DataMailingRegistro,103),'-','') [CODIGO_MAILING],
--A.[NOME_CAMPANHA], 
...
TRS.Descricao [TIPO_COND_SEGURADO],
ISNULL(SA.DataNascimento, A.DataNascimento) [DATA_NASC],
SA.Ano [ANO_MODELO],
...
OUTER APPLY (
				...
				WHERE PC.CodCampanha = 'AQAUTSIA20160301' ----Lincoln(20160316): incluido o filtro codigo de campanha
				...
				, B.CPF
				, B.DataNascimento
				, ACE.TelefoneEmail Email
				, CASE WHEN  LEN(LTRIM(RTRIM(ACT.TelefoneEmail))) >=10 THEN left(LTRIM(RTRIM(ACT.TelefoneEmail)),2) ELSE NULL END [DDDTelefone]
				...
				ON ACT.ID = B.IDTelefoneEfetivo AND ACT.TipoContato = 'Telefone'	
				LEFT JOIN Dados.AtendimentoContatos ACE
				ON ACE.ID = B.IDTelefoneEfetivo AND ACE.TipoContato = 'Email'	
				WHERE B.CPF = A.CPF	 
			) PC	
...
OUTER APPLY (
				 SELECT TOP 1 
				 SA.IDTipoSeguro, SA.DataNascimento, SA.NumeroCalculo, SA.ValorFIPE, SA.NomeCliente, SA.TipoPessoa, SA.IDSexo,
...
, 'AQUISIÇÃO AUTO - RETORNO M12_MS' AS NomeCampanha
, 'AQAUTOMSAA20160301' AS CodigoCampanha
, 'AQAUTOMSAA20160301_' + REPLACE(Convert(date,@DataMailingRegistro,103),'-','') [CODIGO_MAILING]
, PC.NomeCliente
, SA.CPFCNPJ
...
OUTER APPLY (
				...
				FROM Mailing.MailingAutoKPN PC WITH(NOLOCK) 
				WHERE PC.CodCampanha = 'AQAUTSIA20160301' ----Lincoln(20160316): incluido o filtro codigo de campanha
				AND SA.CPFCNPJ_NOFORMAT=PC.CPF 
				AND PC.DataCalculo >= DATEADD(DD,-15 -@SomaDiaInicio,@DataMailing)
...
WHERE MAK.DataRefMailing = DATEADD(DD, -3 -@SomaDiaInicio , @DataMailingRegistro)
AND TS.Descricao IN ('Seguro Novo','Novo','Renovação Congênere')
AND EXISTS		(
					SELECT ID 
					FROM [Mailing].[MailingAutoKPN] AS MAK
					WHERE MAK.CodCampanha = 'AQAUTSIA20160301' ----Lincoln(20160316): incluido o filtro codigo de campanha
...
AND NOT EXISTS (	
					SELECT ID
					FROM Dados.Atendimento AS C2 
					WHERE SA.CPFCNPJ_NOFORMAT = C2.CPF_NOFORMAT
					AND (C2.Arquivo LIKE '%SSIS_DEVOLUÇÃO_PAR_%_CAIXA SEGUROS.txt' OR C2.NomeCampanha = 'AQAUTSIA20160301')
...
-------------------------------------------------------------------------------------------------------------------------------------------
--Alimenta o mailing fazendo transformação de linha (dos contatos) em coluna, e removendo os clientes exclusivos do resultado - FIM
-------------------------------------------------------------------------------------------------------------------------------------------	
...
OUTER APPLY (	
				SELECT TOP 1
				T1.Telefone Telefone,
				ISNULL(T1.LINHATE,0) LINHATE,
				CASE WHEN  T1.Telefone IS NULL THEN NULL ELSE CASE WHEN  T1.OrdemTelefone = 1 THEN 'FIXO' ELSE 'MÓVEL' END END TipoTelefone
				FROM CTE2 T1
				WHERE T1.CPF = A.CPF
				AND T1.Telefone IS NOT NULL
				ORDER BY T1.OrdemTelefone, T1.LINHATE, T1.Telefone
			) T1

OUTER APPLY (	
				SELECT TOP 1 
				T2.Telefone Telefone,
				T2.LINHATE,
				CASE WHEN  T2.Telefone IS NULL THEN NULL ELSE CASE WHEN  T2.OrdemTelefone = 1 THEN 'FIXO' ELSE 'MÓVEL' END END TipoTelefone
				FROM CTE2 T2
				WHERE T2.CPF = A.CPF
...
OUTER APPLY (	
				SELECT TOP 1
				T3.Telefone Telefone,
				T3.LINHATE, 
				CASE WHEN  T3.Telefone IS NULL THEN NULL ELSE CASE WHEN  T3.OrdemTelefone = 1 THEN 'FIXO' ELSE 'MÓVEL' END END TipoTelefone
				FROM CTE2 T3
				WHERE T3.CPF = A.CPF
...
OUTER APPLY (	SELECT TOP 1
				T4.Telefone Telefone,
				T4.LINHATE,
				CASE WHEN  T4.Telefone IS NULL THEN NULL ELSE CASE WHEN  T4.OrdemTelefone = 1 THEN 'FIXO' ELSE 'MÓVEL' END END TipoTelefone
				FROM CTE2 T4
				WHERE T4.CPF = A.CPF
...
AS 
(
SELECT DISTINCT CPF_CNPJ_CLIENTE COLLATE Latin1_General_CI_AS AS CPF 
FROM OBERON.[COL_MULTISEGURADORA].[dbo].[Tabela_Mailing_Diario_MS] 
WHERE Codigo_campanha = 'AQAUTOMSAA20160301' ----<> 'CGAUTCOL20151215' ----Lincoln(20160316): incluido o filtro codigo de campanha
AND Data_inclusao_calculo > DATEADD(DD,-60,@DataMailingRegistro) --and Data_inclusao_calculo < '2015-11-04'

UNION

SELECT DISTINCT CPF 
FROM Mailing.MailingAutoMS 
WHERE DataRefMailing >= DATEADD(DD, -60 -@SomaDiaFim, @DataMailingRegistro) --and DataRefMailing < '2015-11-04'
AND CODIGO_CAMPANHA = 'AQAUTOMSAA20160301' ----Lincoln(20160316): incluido o filtro codigo de campanha

UNION

SELECT DISTINCT  SUBSTRING(CPF,1,3) + '.' + SUBSTRING(CPF,4,3) + '.' + SUBSTRING(CPF,7,3) + '-' + SUBSTRING(CPF,10,2)
...
as 
(
select CPFCNPJ as CPF, DDDResidencial as DDD, TelefoneResidencial as Telefone
from #MUNDOCAIXA MC 
where TelefoneResidencial is not null  --and MC.Empresa ='Enriquecimento'

UNION ALL

select CPFCNPJ, DDDCelular, TelefoneCelular 
from #MUNDOCAIXA MC 
where TelefoneCelular is not null --and MC.Empresa ='Enriquecimento'

UNION ALL

select CPFCNPJ, DDDComercial, TelefoneComercial 
from #MUNDOCAIXA MC 
where TelefoneComercial is not null --and MC.Empresa ='Enriquecimento'

UNION ALL

select CPFCNPJ,
try_parse(ddd as float) as DDD,
try_parse(telefone as float) as Telefone
from (	SELECT 
		CPFCNPJ,
		replace(replace(replace(DDDFax,'-',''),' ',''),'.','') AS DDD,
		replace(replace(replace(TelefoneFax,'-',''),' ',''),'.','')  as Telefone
		from #MUNDOCAIXA MC
		where TelefoneFax is not null AND TelefoneFax <> 'NULL' and TelefoneFax <> '' AND TelefoneFax <> 'NULL' AND tELEFONEFAX IS NOT NULL
	 ) x 
where try_parse(ddd as float) is not null and try_parse(telefone as float)is not null--and MC.Empresa ='Enriquecimento'
)
select 
...
DDD,
CAST(CAST(Telefone AS BIGINT) AS VARCHAR(20)) AS  Telefone,
...
OUTER APPLY (select DDD,Telefone from #Temp_TelMC AS T WHERE T.CPF=A.CPF AND T.Ordem=1) AS TEL1
...
AND Email not like '%@caixa.gov.br'
AND (([TIPO_CLIENTE] NOT like '%PARENTE DO ECONOMIÁRIO (PAIS E FILHOS)%' 
		AND [TIPO_CLIENTE] NOT like '%CÔNJUGE DO ECONOMIÁRIO%'
		AND [TIPO_CLIENTE] NOT like '%AUTO EXCLUSIVO%')	OR [TIPO_CLIENTE] IS NULL)
...
AND NOT EXISTS (	----Exclui os registros que possuem status de proposta efetivada dentro do período de 7 dias
					SELECT SA.CPFCNPJ
					FROM Dados.SimuladorAuto AS SA
...
UPDATE Mailing.MailingAutoMS 
SET RegraAplicada = T.RegraAplicada
FROM Mailing.MailingAutoMS AS M 
INNER JOIN [ClientesM12_TEMP] AS T
ON M.CPF = T.CPF
WHERE DataRefMailing = @DataMailingRegistro

UPDATE Mailing.MailingAutoMS 
SET RegraAplicada = 'Não abordado' 
WHERE DataRefMailing = @DataMailingRegistro
...
SELECT @TotalQuarentena= COUNT(DISTINCT CPF) 
FROM #Temp_FINAL 
WHERE CPF COLLATE Latin1_General_CI_AS IN (SELECT * FROM #TempCOL)

SELECT @TotalTelefoneNull = COUNT(DISTINCT CPF) 
FROM #Temp_FINAL
WHERE Telefone1 IS NULL

SELECT @TotalMotos = COUNT(*)
		 ...
		 OR
			(
				 ([TIPO_CLIENTE] like '%PARENTE DO ECONOMIÁRIO (PAIS E FILHOS)%'
				  OR [TIPO_CLIENTE] like '%CÔNJUGE DO ECONOMIÁRIO%'
				  OR [TIPO_CLIENTE] like '%AUTO EXCLUSIVO%')
...
@CodigoMailing = Codigo_Mailing,
@NomeCampanha = Nome_campanha 
FROM Mailing.MailingAutoMS 
WHERE DataRefMailing = @DataMailingRegistro
AND Nome_Campanha = 'AQUISIÇÃO AUTO - RETORNO M12_MS' --'AQUISICAO_SEGURO AUTO_MS' 
GROUP BY Codigo_Mailing, Nome_campanha
...
----Lincoln(20160316): incluido o filtro codigo de campanha
DECLARE @NaoCotados VARCHAR(10) = (SELECT ISNULL(COUNT(*),0) AS NCotados FROM mailing.mailingautoms WHERE datarefmailing = @DataMailingRegistro AND CODIGO_CAMPANHA = 'AQAUTOMSAA20160301' AND DataEnvioMailing IS NULL)
...
UPDATE [ControleDados].[LogMailing]
SET QuantidadeEconomiarios = @TotalEconomiarios, QuantidadeMotos = @TotalMotos, TextoVariavel = @Texto
WHERE Campanha = 'AQUISIÇÃO AUTO - RETORNO M12_MS' --'AQUISICAO_SEGURO AUTO_MS' 
AND DataRefMailing = @DataMailingRegistro 
