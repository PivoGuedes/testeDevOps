CREATE PROCEDURE [Mailing].[proc_GeraMailing_MS_Recusados](@DataMailing DATE /*A data será subtraida de um para a correta referência do mailing*/ )
AS
...
BEGIN 
	...
	--DECLARE @DataMailing DATE ='2015-11-02'
	...
	WHERE DataRefMailing=@DataMailing AND Nome_Campanha='AQAUTORESTMS20150508' GROUP BY Codigo_Mailing, Nome_campanha

	EXEC [Mailing].[proc_RegistraLogMailing_MS] @DataMailing=@DataMailing, @Empresa='SistemasSeguros', @NomeArquivo=@CodigoMailing, @QtdREgistros=@TotalRegistros, @Campanha=@NomeCampanha
	--PRINT @CodigoMailing
	...
	DECLARE @NaoCotados VARCHAR(10) = (SELECT ISNULL(COUNT(*),0) AS NCotados FROM mailing.mailingautoms WHERE datarefmailing=@DataMailing AND DataEnvioMailing IS NULL)
	...
	WITH C AS (
		SELECT ISNULL(RegraAplicada,'Não identificado') AS RegraAplicada, count(*) AS QtdLeads FROM mailing.mailingautoms WHERE datarefmailing=@DataMailing GROUP BY RegraAplicada
	)
	SELECT * FROM C;
	...
	--@TotalCPFInicio
	UPDATE [ControleDados].[LogMailing] SET QuantidadeEconomiarios=@TotalEconomiarios, QuantidadeMotos=@TotalMotos, TextoVariavel=@Texto WHERE Campanha='AQAUTORESTMS' AND DataRefMailing=@DataMailing
