...
--ALTER PROCEDURE [Mailing].[proc_GeraMailing_MS_Novo](@DataMailing DATE /*A data será subtraida de um para a correta referência do mailing*/ )
CREATE PROCEDURE [Mailing].[proc_GeraMailing_MS_Novo]
AS

BEGIN 
	
	DECLARE @DataMailing DATE ='2015-11-18'
		        ...
		        (
					...
					NOT EXISTS --Sem negócio fechado
								...
								(
									...
									WHERE B.DataArquivoCalculada >= DATEADD(DD,(-3 -@SomaDiaInicio),@DataMailing) --and B.DataArquivoCalculada < DATEADD(DD,0, @DataMailing)
									AND B.CPF = A.CPF
	...
	OUTER APPLY (
						 ...
						 WHERE EXISTS (SELECT * FROM CTE CTE WHERE  PC.CPF = A.CPF_NOFORMAT) AND PC.DataCalculo >= DATEADD(DD,-15 -@SomaDiaInicio,@DataMailing)
						 ...
						 UNION ALL
						 ---- TRECHO INCLUÍDO EM 12/11/2015 por André Anselmo, a pedido do Marcus Reis. Enriquecimento dos leads com dados de contato incluídos na tabela mundo caixa.
						SELECT MC.Nome COLLATE SQL_Latin1_General_CP1_CI_AI AS Nome	, RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI AS CPFCNPJ, MC.DataNascimento COLLATE SQL_Latin1_General_CP1_CI_AI AS DataNascimento,
								MC.emailpessoal COLLATE SQL_Latin1_General_CP1_CI_AI AS Email,CAST(MC.DDDCelular AS VARCHAR(3)) AS DDDCelular, cast(cast(MC.TelefoneCelular as bigint) as varchar(20)) AS TelefoneCelular
						FROM DBM.[DBM_MKT].[dbo].[MUNDO_CAIXA] MC 
						WHERE EXISTS (SELECT * FROM CTE CTE WHERE A.CPF_NOFORMAT = RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI)
						AND MC.Empresa ='Enriquecimento' 

						 UNION ALL


						SELECT MC.Nome COLLATE SQL_Latin1_General_CP1_CI_AI AS Nome	, RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI AS CPFCNPJ, MC.DataNascimento COLLATE SQL_Latin1_General_CP1_CI_AI AS DataNascimento,
								MC.emailpessoal COLLATE SQL_Latin1_General_CP1_CI_AI AS Email,CAST(MC.DDDResidencial AS VARCHAR(3)) AS DDDResidencial, cast(cast(MC.TelefoneResidencial as bigint) as varchar(20)) AS TelefoneResidencial
						FROM DBM.[DBM_MKT].[dbo].[MUNDO_CAIXA] MC 
						WHERE EXISTS (SELECT * FROM CTE CTE WHERE A.CPF_NOFORMAT = RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI)
						AND MC.Empresa ='Enriquecimento' 
		...
		OUTER APPLY 
			(
					...
					SELECT  PC.Nome, PC.CPF, PC.DataNascimento,PC.Email Email, 
							...
							CASE WHEN  LEN(LTRIM(RTRIM(PC.Telefone1))) >=10 THEN RIGHT(LTRIM(RTRIM(PC.Telefone1)), LEN(LTRIM(RTRIM(PC.Telefone1))) - 2) ELSE NULL END [Telefone] 
					FROM Mailing.MailingAutoKPN PC WITH(NOLOCK) 
					WHERE SA.CPFCNPJ_NOFORMAT=PC.CPF AND PC.DataCalculo >= DATEADD(DD,-15 -@SomaDiaInicio,@DataMailing)
					...
					UNION ALL
						---- TRECHO INCLUÍDO EM 12/11/2015 por André Anselmo, a pedido do Marcus Reis. Enriquecimento dos leads com dados de contato incluídos na tabela mundo caixa.
					SELECT MC.Nome COLLATE SQL_Latin1_General_CP1_CI_AI AS Nome	, RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI AS CPFCNPJ, MC.DataNascimento COLLATE SQL_Latin1_General_CP1_CI_AI AS DataNascimento,
							MC.emailpessoal COLLATE SQL_Latin1_General_CP1_CI_AI AS Email, MC.DDDCelular , cast(cast(MC.TelefoneCelular as bigint) as varchar(20)) as TelefoneCelular
					FROM DBM.[DBM_MKT].[dbo].[MUNDO_CAIXA] MC 
					WHERE EXISTS (SELECT * FROM Mailing.MailingAutoKPN AS MAK WHERE MAK.DataRefMailing >= DATEADD(DD,-15 -@SomaDiaInicio,@DataMailing) AND MAK.CPF= RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI)
						AND MC.Empresa ='Enriquecimento' 

					UNION ALL
						---- TRECHO INCLUÍDO EM 12/11/2015 por André Anselmo, a pedido do Marcus Reis. Enriquecimento dos leads com dados de contato incluídos na tabela mundo caixa.
					SELECT MC.Nome COLLATE SQL_Latin1_General_CP1_CI_AI AS Nome	, RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI AS CPFCNPJ, MC.DataNascimento COLLATE SQL_Latin1_General_CP1_CI_AI AS DataNascimento,
							MC.emailpessoal COLLATE SQL_Latin1_General_CP1_CI_AI AS Email, MC.DDDResidencial , cast(cast(MC.TelefoneResidencial as bigint) as varchar(20)) as TelefoneResidencial
					FROM DBM.[DBM_MKT].[dbo].[MUNDO_CAIXA] MC 
					WHERE EXISTS (SELECT * FROM Mailing.MailingAutoKPN AS MAK WHERE MAK.DataRefMailing >= DATEADD(DD,-15 -@SomaDiaInicio,@DataMailing) AND MAK.CPF= RIGHT('000000000' + MC.CPFCNPJ,11) COLLATE SQL_Latin1_General_CP1_CI_AI)
						AND MC.Empresa ='Enriquecimento' 
				...
				AND EXISTS		(
						SELECT ID FROM [Mailing].[MailingAutoKPN] AS MAK
						WHERE MAK.cpf=sa.cpfcnpj_noformat
	...
	;  WITH Temp AS (
		SELECT DISTINCT CPF_CNPJ_CLIENTE COLLATE Latin1_General_CI_AS AS CPF FROM OBERON.[COL_MULTISEGURADORA].[dbo].[Tabela_Mailing_Diario_MS] WHERE Data_inclusao_calculo> DATEADD(DD,-60,@DataMailingRegistro) and Data_inclusao_calculo < '2015-11-17'
		UNION
		SELECT DISTINCT CPF FROM Mailing.MailingAutoMS WHERE DataRefMailing>=DATEADD(DD, -60 -@SomaDiaFim, @DataMailingRegistro) and DataRefMailing < '2015-11-17'
		UNION
	...
	SELECT
			...
			DataMailing
	into dbo.Temp_Mailing_TesteAndre
	FROM #Temp_FINAL A
	OUTER APPLY (SELECT TOP 1 TipoVeiculo
				FROM Dados.TabelaFIPE
				WHERE CodigoCompleto=A.COD_FIPE
