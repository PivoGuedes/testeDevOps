...
CREATE PROCEDURE [Mailing].[proc_GeraMailing_Flex_RetornoM12](@DataMailing DATE)
AS
...
WHERE MS.Data_inclusao_Calculo >= DATEADD(DD, -@SomaDia, @DataMailing) AND MS.Data_inclusao_Calculo <= @DataMailing
AND MS.SITUACAO = '2'
	
AND EXISTS		(	-- Este trecho de código filtra os registros marcados para o mailing atendente.
					SELECT * 
					FROM Mailing.MailingAutoMS AS M
					WHERE M.DataRefMailing = MS.Data_inclusao_Calculo
					AND M.CPF = MS.CPF_CNPJ_CLIENTE COLLATE Latin1_General_CI_AS
					AND M.CODIGO_CAMPANHA = 'AQAUTOMSAA20160301' ----Nome_Campanha = 'AQUISICAO_SEGURO AUTO_MS'
				)

AND NOT EXISTS (	
					SELECT *
					FROM OBERON.[COL_MULTISEGURADORA].[dbo].[Tabela_Mailing_Diario_MS] B
					WHERE B.CODIGO_CAMPANHA = 'AQAUTOMSAA20160301'
					AND B.Data_inclusao_Calculo IS NULL
				)

AND NOT EXISTS (	 ---- Busca os CPF'S cadastrados no blacklist PROCON no banco FENAE
					 SELECT CPF 
					 FROM OBERON.[FENAE].[dbo].[BlacklistProcon] AS B
					 WHERE B.CPF = REPLACE(REPLACE(REPLACE(RTRIM(LTRIM(MS.CPF_CNPJ_CLIENTE)), '.', ''), '-', ''), ' ', '') COLLATE SQL_Latin1_General_CP1_CI_AI
				)
