CREATE PROCEDURE [Mailing].[proc_GeraMailing_Flex_Recusados](@DataMailing DATE)
AS
...
WHERE 
	...
	AND EXISTS (	-- Este trecho de código filtra os registros marcados para o mailing atendente.
					...
					WHERE M.DataRefMailing=MS.Data_inclusao_Calculo AND M.CPF=MS.CPF_CNPJ_CLIENTE COLLATE Latin1_General_CI_AS 
...
AND NOT EXISTS 
 ...
 SELECT CPF FROM OBERON.[FENAE].[dbo].[BlacklistProcon] AS B WHERE B.CPF = REPLACE(REPLACE(REPLACE(RTRIM(LTRIM(MS.CPF_CNPJ_CLIENTE)), '.', ''), '-', ''), ' ', '') COLLATE SQL_Latin1_General_CP1_CI_AI
  ...
  AND CODIGO_CAMPANHA <> 'CGAUTCOL20151215'  
--EXEC  Mailing.[proc_GeraMailing_Flex_Recusados] '2014-08-27'
