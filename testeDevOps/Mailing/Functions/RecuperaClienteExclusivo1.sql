...
CREATE FUNCTION [Mailing].[RecuperaClienteExclusivo1] (@CPF VARCHAR(20))
...
--DECLARE @CPF VARCHAR(20) = '928.999.941-15'
	...
	SELECT DISTINCT PC.Nome, PC.Matricula, PC.CPFCNPJ, /*RP.Codigo [CodigoBloco], RP.Nome [NomeBloco], */ COALESCE(PC.Email, A.EMail) EMail, PC.DataNascimento, PRP.IDAgenciaVenda [IDAgenciaCliente], PRP.IDProduto
	  ...
	  FROM 
			...
			FROM Dados.Funcionario F  WITH(NOLOCK)
		...
		CROSS APPLY
		 ...
		 (
		   SELECT PC.IDProposta, PC.Nome, PC.CPFCNPJ, PC.Matricula, PC.DataNascimento, COALESCE(PC.Email, PC.Emailcomercial) Email, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.DDDResidencial, '(', ''), ')', ''),'.',''),'-',''),' ',''),3), ' ', NULL) DDDTelefone, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.TelefoneResidencial, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL) [Telefone], 1 Prioriza
		   FROM Dados.PropostaCliente PC WITH(NOLOCK) 
		   WHERE PC.CPFCNPJ = @CPF

		   UNION ALL 
		   SELECT PC.IDProposta, PC.Nome, PC.CPFCNPJ, PC.Matricula, PC.DataNascimento, COALESCE(PC.Emailcomercial, PC.Email) Email, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.DDDComercial, '(', ''), ')', ''),'.',''),'-',''),' ',''),3), ' ', NULL) DDDTelefone, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.TelefoneComercial, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL) [Telefone], 1 Prioriza
		   FROM Dados.PropostaCliente PC WITH(NOLOCK)  --WHERE TelefoneResidencial LIKE '%2242438%'
		   WHERE PC.CPFCNPJ = @CPF
    
		   UNION ALL
		   SELECT  PC.IDProposta, PC.Nome, PC.CPFCNPJ, PC.Matricula, PC.DataNascimento, NULL Email, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.DDDFax, '(', ''), ')', ''),'.',''),'-',''),' ',''),3), ' ', NULL) DDDTelefone, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.TelefoneFax, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL) [Telefone], 1 Prioriza
		   FROM Dados.PropostaCliente PC WITH(NOLOCK) 
		   WHERE PC.CPFCNPJ = @CPF
     
		   UNION ALL
		   SELECT  PC.IDProposta, PC.Nome, PC.CPFCNPJ, PC.Matricula, PC.DataNascimento, NULL Email, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.DDDCelular, '(', ''), ')', ''),'.',''),'-',''),' ',''),3), ' ', NULL) DDDTelefone, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(PC.TelefoneCelular, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL) [Telefone], 1 Prioriza 
		   FROM Dados.PropostaCliente PC WITH(NOLOCK)
		   WHERE PC.CPFCNPJ = @CPF
		   ...
		   SELECT NULL IDProposta, MC.Nome COLLATE SQL_Latin1_General_CP1_CI_AI Nome,  CleansingKit.dbo.fn_FormataCPF([CPFCNPJ] COLLATE SQL_Latin1_General_CP1_CI_AI) [CPFCNPJ], Cast(MC.Matricula as varchar(20)) COLLATE SQL_Latin1_General_CP1_CI_AI Matricula, MC.DataNascimento, [EmailPessoal] COLLATE SQL_Latin1_General_CP1_CI_AI [EmailPessoal], Cast(Cast([DDDResidencial] as int) as varchar(3)) COLLATE SQL_Latin1_General_CP1_CI_AI [DDDResidencial], Cast(Cast(REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TelefoneResidencial, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL) as bigint) as varchar(10)) COLLATE SQL_Latin1_General_CP1_CI_AI [TelefoneResidencial], 1 Prioriza
		   FROM DW.[DBM_MKT].[dbo].[MUNDO_CAIXA] MC WITH(NOLOCK)
		   WHERE  MC.CPFCNPJ  = Cast(REPLACE(REPLACE(@CPF,'.',''), '-', '') as nvarchar(255))

		   UNION  ALL  
	   
		   SELECT TOP 1 NULL IDProposta, F.Nome, F.CPF, F.Matricula, F.DataNascimento, F.Email, CASE WHEN LEN(F.DDDCelular) = 9 THEN REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(F.DDDCelular, '(', ''), ')', ''),'.',''),'-',''),' ',''),3), ' ', NULL) ELSE F.DDDCelular END DDDCelular, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(F.Celular, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL), 0 Prioriza
		   FROM Dados.Funcionario F  WITH(NOLOCK)
		   WHERE F.CPF =  @CPF
		   ORDER BY DataAdmissao DESC, DataUltimaSituacaoFuncionario DESC
		   ...
		   SELECT TOP 1 NULL IDProposta, F.Nome, F.CPF, F.Matricula, F.DataNascimento, F.Email, CASE WHEN LEN(F.DDD) = 9 THEN REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(F.DDD, '(', ''), ')', ''),'.',''),'-',''),' ',''),3), ' ', NULL) ELSE F.DDD END DDD, REPLACE(RIGHT(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(F.Telefone, '(', ''), ')', ''),'.',''),'-',''),' ',''),9), ' ', NULL), 0 Prioriza
		   FROM Dados.Funcionario F  WITH(NOLOCK)
